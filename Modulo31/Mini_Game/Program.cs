...
string playerAppearance(int currentFood)
{
    if (player == "(X_X)")
    {
        FreezePlayer();
    }
    return states[currentFood];
}

int isHappy() => (player == "(^-^)") ? 3 : 0;
...
void Move(int movSpeed, bool nondirectionalKey = false)
{
    int lastX = playerX;
    int lastY = playerY;
    ...
    switch (Console.ReadKey(true).Key)
    {
        ...
        default:
            ...
            {
                Console.Clear();
                Console.WriteLine("Nondirectional key detected. Program exiting.");
                Thread.Sleep(2000);
                shouldExit = true;
            }
            break;
...
int foodEaten()
{
    if (playerX > foodX - foods[food].Length && playerX < foodX + foods[food].Length && playerY == foodY)
    {
        Console.Clear();
        ShowFood();
        ChangePlayer(); ;
